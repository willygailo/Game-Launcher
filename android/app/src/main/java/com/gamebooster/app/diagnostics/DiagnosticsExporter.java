...
public final class DiagnosticsExporter {
    ...
    public static List<String> buildSnapshot(String appVersion, String deviceModel,
        ...
        lines.add("Shizuku Shell Privileges: " + (shizukuGranted ? "🛡️ GRANTED (UID 2000 Privileged Shell)" : "⚠️ NOT GRANTED (Restricted Mode)"));
        if (shizukuGranted) {
            try {
        ...
        try {
            ...
            if (!aidlConnected && shizukuGranted) {
                // Auto-trigger binding in background if granted but not yet bound
                ShizukuUserServiceConnector.getInstance().bindService();
            }
        } catch (Throwable ignored) {}
        lines.add("AIDL UserService Bound: " + (aidlConnected ? "🟢 BOUND & ACTIVE (Direct Root/Shell Binder)" : "🟡 RE-CONNECTING / STANDBY"));
        lines.add("Rish Shell Executable: " + (RishManager.isRishAvailable() ? "🟢 READY (Interactive Shell Available)" : "🟡 STANDBY"));
        try {
        ...
        if (context != null) {
            ...
            try {
                boolean webviewBoost = com.gamebooster.app.config.ManualSettingsPreferences.isWebViewBoostEnabled(context);
                com.gamebooster.app.booster.WebViewBoosterChannel.DeviceTier tier =
                        com.gamebooster.app.booster.WebViewBoosterChannel.detectDeviceTier(context);
                File flagFile = new File("/data/local/tmp/webview-command-line");
                boolean flagsExist = flagFile.exists();
                lines.add("⚡ Android WebView & Chromium Acceleration: " + (webviewBoost ? "🟢 ACTIVE [" + tier.name() + "]" : "⚪ OFF"));
                lines.add("   • Target Profile: " + tier.description);
                lines.add("   • Command-Line Flag Status: " + (flagsExist ? "✅ Synchronized (/data/local/tmp/webview-command-line)" : "⚠️ Standby"));
            } catch (Throwable ignored) {
                lines.add("⚡ Android WebView & Chromium Acceleration: ACTIVE");
            }
        }
        lines.add("");
    ...
    /**
     * Clears all crash logs and cached diagnostic report files from internal and external storage.
     */
    public static boolean clearAllDiagnosticsData(Context context) {
        ...
        try {
            ...
            if (cacheDir != null && cacheDir.exists()) {
                File[] files = cacheDir.listFiles((d, name) -> name.startsWith(PREFIX) && name.endsWith(EXTENSION));
                if (files != null) {
                    for (File f : files) {
            ...
            if (extDir != null && extDir.exists()) {
                File[] files = extDir.listFiles((d, name) -> name.startsWith(PREFIX) && name.endsWith(EXTENSION));
                if (files != null) {
                    for (File f : files) {
                        try { f.delete(); } catch (Throwable ignored) {}
                    }
