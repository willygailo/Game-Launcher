...
public class NewStateConfigPatcher {

    private static final String TAG = "NewStateConfigPatcher";

    public static boolean patch(String packageName, int targetFps) {
        if (packageName == null) return false;
        String pkg = packageName.toLowerCase().trim();

        Log.d(TAG, "Patching PUBG: NEW STATE config for " + pkg + " -> " + targetFps + " FPS/Hz");

        String[] configPaths = new String[] {
                "/sdcard/Android/data/" + pkg + "/files/UE4Game/NewState/NewState/Saved/Config/Android/GameUserSettings.ini",
                "/sdcard/Android/data/" + pkg + "/files/UE4Game/NewState/NewState/Saved/Config/Android/UserSettings.json",
                "/data/data/" + pkg + "/files/UE4Game/NewState/NewState/Saved/Config/Android/GameUserSettings.ini"
        };
    ...
    private static boolean patchPath(String path, int targetFps) {
        ...
        if (!checkRes.contains("EXISTS")) {
            ...
            CommandExecutor.executeSystemCommand("sed -i 's/^CustomFrameRateOption=.*/CustomFrameRateOption=5/' " + path);

            ShizukuExecutor.executeShizukuCommand("sed -i 's/^FrameRateLimit=.*/FrameRateLimit=" + String.format("%.6f", (float) targetFps) + "/' " + path);
            ShizukuExecutor.executeShizukuCommand("sed -i 's/^CustomFrameRateOption=.*/CustomFrameRateOption=5/' " + path);
        }
        return true;
    }
